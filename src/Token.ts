...
export class Token extends BaseToken<Erc20> {
    ...
    public async updateBalance(user: string) {
        this.rawBalance = await this.contract.balanceOf(user)
    ...
    public async updateAllowance(user: string, spender: string) {
        this.rawAllowance = await this.contract.allowance(user, spender)
    ...
    public static async init(address: string, chain: Chain): Promise<Token> {
